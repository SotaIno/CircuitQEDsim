...
class CZpulse():
    def __init__(self,Q1,Q2,J,lambda2=0.10,the_f=0.85,tg_param=1.2,steps=1,gatetime=None): #Jは/2piのものを入れる
        self.Nq1=Q1.Nq
        self.Nq2=Q2.Nq
        ...
        self.anh1=Q1.anh
        self.fq2=Q2.f01
        self.wq2=self.fq2*2*pi
        self.anh2=Q2.anh
        self.qFreq20=2*pi*(self.fq1+abs(self.anh2))
        self.J=J
        self.lambda2=lambda2
        self.the_f=the_f
        self.tg_param=tg_param
        self.steps=steps
        ...
        if gatetime==None:
            self.tg=round((self.tg_param/(np.sqrt(2)*self.J)),1)
        else:
            self.tg=gatetime
        self.gatetime=gatetime
        self.t_list=np.linspace(0,self.tg,int(self.tg*self.steps))
    
    def squarepulse(self,t_ini,t_ramp):
        Tg = self.tg
        t_list = np.linspace(0,Tg,int(self.tg*self.steps))
        shape = []
        ...
        for t in t_list:
            if t < t_ini:
                ...
                a = 0 - (0-0.573) / t_ramp*(t-t_ini)
            elif t > Tg - (t_ini):
                a = 0
            elif t > Tg - (t_ramp+t_ini):
                a = 0 - (0-0.573) / t_ramp*(Tg-(t+t_ini))
            else:
                a = 0.573
    ...
    def co_optim(self,taug):

        tau_list=np.linspace(0,taug,int(self.tg*self.steps))
    ...
    def netzerocurrent(self,M=1):
        t_list,freqs=self.netzeropulse()
        current=np.zeros(len(freqs))
        for i,freq in enumerate(freqs):
            if i<=len(freqs)/2:
                current[i]=np.arccos(((freq-abs(self.anh2*(2*pi)))**2/((self.fq2-abs(self.anh2))*(2*pi))**2))/M
            else:
                current[i]=-np.arccos(((freq-abs(self.anh2*(2*pi)))**2/((self.fq2-abs(self.anh2))*(2*pi))**2))/M
        return t_list,current
...
if __name__=='__main__':
    ...
    pulsesystem=CZpulse(Q1,Q2,g,the_f=0.88,lambda2=0.13,gatetime=48)
