...
from FidelityCode import state_fidelity
...
for gt in range(tgstart,tgend+1):
    ...
    tg=pulsesystem.tg
    t_list,squarepulse = pulsesystem.squarepulse(1,5)
    ...
    print(q_state_list[0])

    phasec = PhaseChange(q_state_list)[0]

    fig,axes=plt.subplots(nrows=1,ncols=2,figsize=(10,8))
    ax1=axes[0]
    ax2=axes[1]
    ax1.plot(t_list,adiabaticpulse)
    ax1.hlines(degeneracy_freq,0,tg,color='black',linestyle='dashed')
    ax2.plot(t_list,phasec)
    plt.savefig(iDir+'/hikaku/tg={:.1f}.png'.format(tg))
    plt.close()