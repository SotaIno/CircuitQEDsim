...
class transmon():
    ...
    def calcNthlevelenergy(self, EC, EJ, N, level,nglist=np.linspace(-1,1,101)):
        nthlevel_ene=[]
        for ng in nglist:
            ene = self.chargeQubitHamilonian(EC, EJ, N, ng).eigenenergies()
            nthlevel_ene.append(ene[level]-ene[0])
        return nglist,nthlevel_ene
...
class Tunabletransmon():
    def __init__(self, EC=10, EJmax=0.3, phi=0, f01=None, alpha=None, N=10, Nq=3, Temp=20e-3):
        ...
        if f01 is None and alpha is None:
            ...
            self.N = N
            self.phi = phi
            self.enes = self.calcChargeQubitLevels(self.EC, self.EJmax, self.N, self.phi)
            self.f01 = self.enes[1]
            self.anh = self.enes[2] - 2*self.enes[1]
        else:
            self.anh = - abs(alpha)
            self.f01 = (f01-self.anh)*np.sqrt(abs(np.cos(phi*pi)))+self.anh
            self.enes = [0] + [i*self.f01 + (i-1)*self.anh for i in range(1, N)]
        self.nth_q = Maxwell_Boltzmann_distributon(self.enes, Temp, N)
        self.P0 = iniState1Qsys(Nq, 0, mode='rho')
        self.P1 = iniState1Qsys(Nq, 1, mode='rho')
    ...
    def chargeQubitHamilonian(self, Ec, Ejmax, N, ng, phi):
        
        m = np.diag(4 * Ec * (np.arange(-N,N+1)-ng)**2) + 0.5 * Ejmax *abs(np.cos(phi*pi))* (np.diag(-np.ones(2*N), 1) +
                                                                np.diag(-np.ones(2*N), -1))
        return qt.Qobj(m)
    ...
    def calcChargeQubitLevels(self, EC, EJmax, N, Phi):
        ng_vec = [0]
        ene = np.array([self.chargeQubitHamilonian(EC, EJmax, N, ng, Phi).eigenenergies() for ng in ng_vec])
        return [ene[0][i] - ene[0][0] for i in range(len(ene[0])) ]
    
    def calcNthlevelenergy(self, EC, EJmax, N, level,ng=0,philist=np.linspace(-1,1,101)):
        nthlevel_ene=[]
        for phi in philist:
            ene = self.chargeQubitHamilonian(EC, EJmax, N, ng, phi).eigenenergies()
            nthlevel_ene.append(ene[level]-ene[0])
        return philist,nthlevel_ene
...
class QQ():
    ...
    def __init__(self, Q1, Q2, g12):
        ...
        self.colordict={'00':'b','01':'r','10':'y','11':'m','02':'c','20':'k','12':'#a65628','21':'#ff7f00'}
    ...
    def plotDressedEnergyLevels(self, figname=1,title=None,savepath=None):
        if self.Nq1 == self.Nq2:
            Nq = self.Nq2
        else:
            print('Should be Nq1 = Nq2')
        ...
        if title==None:
            ...
            plt.title(title)
        colordict={'00':'b','01':'r','10':'y','11':'m','02':'c','20':'k','12':'#a65628','21':'#ff7f00'}
        for i in range(Nq):
            for j in range(Nq):
                key = '{}{}'.format(i,j)
                if key == '22':
                ...
                if i < j:#p
                    ...
                    s = 0
                    t = 1
                plt.hlines(ene, s, t,colors=colordict[key])
                plt.text(s, ene+0.4, '|'+key+'>')
